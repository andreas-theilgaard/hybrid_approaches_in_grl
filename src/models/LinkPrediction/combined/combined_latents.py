...
def test_joint_with_emb_combined_collab(
    shallow, deep, predictor, split_edge, x, adj_t, evaluator, batch_size=65536
):
    shallow.eval()
    deep.eval()
    predictor.eval()
...
def evaluate_emb(type_, shallow, deep, predictor, deep_data, batch_size, split_edge, config):
    shallow.eval()
    deep.eval()
    predictor.eval()

    with torch.no_grad():
        W = deep(deep_data.x, deep_data.edge_index)
        Z = shallow.weight.data.to(config.device)
        concat_embeddings = torch.cat([Z, W], dim=-1)

        pos_test_edge = split_edge[type_]["edge"].to(config.device)
        if type_ != "train":
            neg_test_edge = split_edge[type_]["edge_neg"].to(config.device)

        pos_preds = []
        for perm in DataLoader(range(pos_test_edge.size(0)), batch_size):
            edge = pos_test_edge[perm].t()
            deep_o = predictor(concat_embeddings[edge[0]], concat_embeddings[edge[1]]).squeeze().cpu()
            pos_preds += [torch.sigmoid(deep_o)]
        ...
        if type_ != "train":
            ...
            for perm in DataLoader(range(neg_test_edge.size(0)), batch_size):
                edge = neg_test_edge[perm].t()
                deep_o = predictor(concat_embeddings[edge[0]], concat_embeddings[edge[1]]).squeeze().cpu()
                neg_preds += [torch.sigmoid(deep_o)]
...
def test_joint_with_emb_combined(
    shallow, deep, predictor, split_edge, x, adj_t, evaluator, batch_size=65536, config=None, data_splits=None
):
    if config.dataset.dataset_name in ["ogbl-collab", "Flickr", "Twitch"]:
        return test_joint_with_emb_combined_collab(
            shallow=shallow,
            deep=deep,
            predictor=predictor,
            split_edge=split_edge,
...
def fit_combined_latents(config, dataset, training_args, Logger, log, seeds, save_path):
    ...
    for counter, seed in enumerate(seeds):
        ...
        Logger.end_run()
    Logger.save_results(save_path + "/link_combined_latents.json")
    if "save_to_folder" in config:
        ...
        Logger.save_results(additional_save_path + f"/link_combined_latents_{training_args.deep_model}.json")
